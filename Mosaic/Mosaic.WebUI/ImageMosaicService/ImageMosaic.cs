...
namespace ImageMosaicService
{
    public class ImageMosaic
    {
        public ImageMosaicResponse Generate(ImageMosaicRequest request)
        {
            ...
            var mosaic = mosaicGenerator.Generate(request.Master.FilePath, request.Tiles.ToList(), request.Random, request.TileWidth, request.TileHeight, request.ColourBlended, request.Enhanced, request.EdgeDetection, request.Edges);
